      ...
      <section className="bg-card border border-border rounded-lg p-3 overflow-x-auto space-y-4">
        ...
        {!canManageRoles ? (
          <div className="text-xs border border-warning/40 bg-warning/10 rounded p-2 text-muted-foreground">
            Nie masz uprawnienia <span className="font-mono">admin.roles.write</span> w tej firmie — role sa tylko do podgladu.
          </div>
        ) : null}
        ...
        {createRoleMutation.isError ? (
          <div className="text-xs text-destructive">
            {mutationErrorMessage(createRoleMutation.error, 'Nie udalo sie utworzyc roli.')}
          </div>
        ) : null}
        <button
          type="button"
