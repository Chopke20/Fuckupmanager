...
- **Monorepo i warstwy**
  ...
  - **Dokumentacja i plany**: katalog `docs/` zawiera:
    ...
    - `PLAN_OFERTA_WSPOLDZIELONA_TOINEN.md` – zewnętrzny edytor oferty dla Toinen Music (`SHARED_OFFER` / `TOI-…`, `/so/:token`),
    ...
    - `deploy-quick.md` – codzienna checklista release (`build -> commit -> push -> verify SHA/version`).
...
- **Główne domeny biznesowe (backend + frontend)**
  ...
  - **Auth, konta, role i uprawnienia (`auth`)**
    ...
    - Publiczne endpointy (`/api/auth/login`, `forgot-password`, `reset-password`, `accept-invite`) oraz panel admina (`/admin`) z zarządzaniem użytkownikami/rolami/auditem i **backupem bazy danych** (uprawnienie `admin.backup`, endpoint `GET /api/auth/admin/backup` — pobiera backup PostgreSQL per-firma jako `.dump`; opcjonalnie `BACKUP_DIR` zapisuje kopię na serwerze).
  - **Integracje AI (`ai`)**
...
- **Powiązane dokumenty i szersza specyfikacja**:
  ...
  - Każda zmiana w generatorze oferty PDF powinna:
    - zachować spójność pól `orderNumber` / `orderYear` / `offerVersion` / `offerNumber` (wyświetlanie: `ORD-YY-NNNN`, eksport oferty: `OFR-YY-NNNN-v#`) i nazwę pliku `Oferta-{offerNumber}.pdf`,
